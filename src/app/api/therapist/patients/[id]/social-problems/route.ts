...
import { serializeSocialProblem } from "@/lib/server/resources";
...
export async function GET(
  ...
  const [socialProblems, total] = await Promise.all([
    prisma.socialProblem.findMany({
      where,
      orderBy: [{ date: "desc" }, { time: "desc" }],
      skip,
      take: perPage,
    }),
    prisma.socialProblem.count({ where }),
  ]);

  return jsonResponse(
    buildPaginatedResponse(
      request,
      socialProblems.map(serializeSocialProblem),
      total,
      page,
      perPage
    )
