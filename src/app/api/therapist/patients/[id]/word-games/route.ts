...
import { serializeWordGame } from "@/lib/server/resources";
...
export async function GET(
  ...
  const [wordGames, total] = await Promise.all([
    prisma.wordGame.findMany({
      where,
      orderBy: [{ date: "desc" }, { time: "desc" }],
      skip,
      take: perPage,
    }),
    prisma.wordGame.count({ where }),
  ]);

  return jsonResponse(
    buildPaginatedResponse(
      request,
      wordGames.map(serializeWordGame),
      total,
      page,
      perPage
    )
